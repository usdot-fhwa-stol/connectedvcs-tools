...
[Visual Studio Code](https://code.visualstudio.com/) is a free IDE that works well for connectedvcs-tools development. This guide covers setting up SonarLint for real-time code quality feedback against the connectedvcs-tools SonarCloud project.
...
- **SonarLint** (`SonarSource.sonarlint-vscode`) - code quality and linting tool that provides feedback on best practices and security issues. Connected to the connectedvcs-tools SonarCloud project so findings match what runs in CI.
...
The **Connection ID** must match exactly. The workspace `.vscode/settings.json` references this name to bind the project to the `usdot-fhwa-stol_connectedvcs-tools` SonarCloud project.
