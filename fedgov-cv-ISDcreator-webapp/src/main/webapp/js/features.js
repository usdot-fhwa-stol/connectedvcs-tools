...
async function placeComputedLane(newDotFeature, lanes, vectors, laneMarkers, laneWidths, computingLane, computedLaneSource, laneTypeOptions, typeAttributeNameSaved, controls) {
	...
	if(computingLane) {
		...
		var offsetZ = Math.round((newZ - computedLaneSource.get("elevation")[0].value) * 100);
...
function connectComputedDots(i, points, initialize, lanes, laneMarkers){
	...
	if (initialize) {
		...
		computedLaneFeat.setProperties({
			...
			"laneInfoDaySelection": laneMarkerFeatures[m].get("laneInfoDaySelection"),
			"laneInfoTimePeriodType": laneMarkerFeatures[m].get("laneInfoTimePeriodType"),
			"laneInfoTimePeriodValue": laneMarkerFeatures[m].get("laneInfoTimePeriodValue"),
			"laneInfoTimePeriodRange": laneMarkerFeatures[m].get("laneInfoTimePeriodRange"),
